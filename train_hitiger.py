...
def train(epoch):
    ...
    for batch_idx, (data, target) in enumerate(train_loader):
        ...
        if trainning_data_visualization:
            images = data.numpy()
            for image in images:
                image = (image.transpose((1,2,0))*255).astype('uint8')[:,:,[2,1,0]]
                cv2.imshow('1',image)
                key = cv2.waitKey(0)
                if key == 27:
                    exit()
        ...
        seen_batch = epoch * math.ceil(len(train_loader.dataset) / float(batch_size) ) + niter
        training_iters.append(seen_batch)
        if len(training_losses) % 50 == 0:
            summary.add_scalar('training loss',np.mean(training_losses[-50:]),seen_batch)
    ...
    for batch_idx, (data, target) in enumerate(test_loader):
        ...
        for box_pr, target in zip([all_boxes], [target[0]]):
            ...
            for k in range(num_gts):
                ...
                # R_gt, t_gt = pnp(np.array(np.transpose(corners3D[:3, :]), dtype='float32'),  corners2D_gt, np.array(internal_calibration, dtype='float32'))
                # R_pr, t_pr = pnp(np.array(np.transpose(corners3D[:3, :]), dtype='float32'),  corners2D_pr, np.array(internal_calibration, dtype='float32'))
...
if __name__ == "__main__":
    ...
    trainning_data_visualization = False


    # Parse configuration files
    parser = argparse.ArgumentParser(description='SingleShotPose')
    parser.add_argument('--datacfg', type=str, default='cfg/hitiger.data') # data config
    ...
    summary = SummaryWriter('log/hitiger')
    ...
    bg_file_names = get_all_files('/home/lls/yangyang/singleshotpose-master/VOCdevkit/VOC2012/JPEGImages')
    ...
    # vertices             = np.c_[np.array(mesh.points), np.ones((len(mesh.points), 1))].transpose()
    # corners3D            = get_3D_corners(vertices)     # 4x8

    corners              = data_config.box_6d().hitiger[1:,:]  # 8x3
    corners3D            = np.concatenate((np.transpose(corners)/1000., np.ones((1,8)) ), axis=0) # 4x8
    ...
    for epoch in range(init_epoch, max_epochs): 
        ...
        if (epoch % 10 == 0) and (epoch > 15): 
            ...
            model.save_weights('%s/model%d.weights' % (backupdir,epoch))
