...
def get_3D_corners(vertices):
    ...
    corners = np.array([[min_x, min_y, min_z],
                        ...
                        [max_x, max_y, min_z],
                        [max_x, max_y, max_z]])
    corners = np.vstack((np.array([[(min_x+max_x)/2.,(min_y+max_y)/2.,(min_z+max_z)/2.]]),corners))
...
def pnp(points_3D, points_2D, cameraMatrix):
    try:
        ...
        # distCoeffs = np.zeros((9, 1), dtype='float32') 
...
def get_region_boxes(output, num_classes, num_keypoints, only_objectness=1, validation=True):
    ...
    # grid_x    = torch.linspace(0, w-1, w).repeat(h,1).repeat(batch*anchor_dim, 1, 1).view(batch*anchor_dim*h*w)       # 只在CPU上测试需要用到
    # grid_y    = torch.linspace(0, h-1, h).repeat(w,1).t().repeat(batch*anchor_dim, 1, 1).view(batch*anchor_dim*h*w)   # 只在CPU上测试需要用到
